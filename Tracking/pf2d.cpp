...
//	printf("Obj[%6.2lf %6.2lf] Est[%6.2lf %6.2lf]\n",l0_obj*cos(thetal0_obj),l0_obj*sin(thetal0_obj),l0*cos(thetal0),l0*sin(thetal0));
	...
	for(i=0;i<2;i++) {
		target[i] = new double [Nt];
		if(target[i]==NULL) exit(-1);
	}
	for(i=0;i<2;i++)
		for(t=0;t<Nt;t++)	target[i][t]=0; 

	estimate = new double *[2];
	if(estimate==NULL) exit(-1);
	for(i=0;i<2;i++) {
		estimate[i] = new double [Nt];
		if(estimate[i]==NULL) exit(-1);
	}
	for(i=0;i<2;i++)
		for(t=0;t<Nt;t++)	estimate[i][t]=0; 
}
...
pf2d::~pf2d()
{
	...
	// Background Index
	for(i=0;i<2;i++) delete target[i],estimate[i];
...
void pf2d::get_yn(void)
{
	...
	target[0][0] = l_obj*cos(thetal_obj);
	target[1][0] = l_obj*sin(thetal_obj);
...
//		if(thetal_obj>2*pi-ang_error || thetal_obj<ang_error) thetal_obj=2*pi;
		...
		// Save to file
		target[0][t] = l_obj*cos(thetal_obj);
		target[1][t] = l_obj*sin(thetal_obj);
		fprintf(fp,"%lf %lf\n",target[0][t],target[1][t]);
	}
...
//			dr[i] += 0.1*gause_noise();
	...
	for(t=0;t<Nt;t++) {
		estimate[0][t] = l_est[t]*cos(thetal_est[t]);
		estimate[1][t] = l_est[t]*sin(thetal_est[t]);

		fprintf(fp,"%lf %lf\n",estimate[0][t],estimate[1][t]);
	}
	fclose(fp);
} 
